...
export class CategoryEditComponent implements OnInit, OnDestroy {
		...
		this.activatedRoute.params.subscribe(params => {
			...
			if (id && id > 0) {
				this.store.pipe(
					...
					this.imageUrl = this.domainURL.domain + '/images/categories/' + result.image;
	...
	*/
	...
	onSumbit(withBack: boolean = false) {
		...
		*/
		...
		this.updateCategory(dataForm, withBack);
		return;
	...
	*/
	 ...
	 * @param withBack: boolean
	 */
	updateCategory(formData, withBack: boolean = false) {
		this.loadingSubject.next(true);
		...
		return this.router.navigate(['vp-admin/catalog/categories']);
	}
	...
	getComponentTitle() {
		...
		result = `Edit category - ${this.category.name} ${this.category.description}`;
		return result;
	}
