...
export class UserEditComponent implements OnInit, OnDestroy {
	...
	createForm() {
		...
		this.userForm = this.userFB.group({
			...
			role: [this.user.roles.toString(), Validators.required],
	...
	onSumbit(withBack: boolean = false) {
		...
		const user = find(this.alluser, function(item: User) {
			return (item.email.toLowerCase() === controls['email'].value.toLowerCase());
		});

		if (user) {
			this.hasEmailErrors = true;
			return;
		}
	...
	onSave(withBack: boolean = false) {
		...
		const editedUser = this.prepareUser1();
		this.updateUser(editedUser, withBack);
	}
	...
	prepareUser(): User {
		...
		_user.address = this.user.address;
