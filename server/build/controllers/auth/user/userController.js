...
class UserController {
    list(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const user = yield database_1.default.query('SELECT a.*, b.title as title FROM vh_user_admin a LEFT JOIN vh_user_roles_admin b ON a.roles = b.id');
    ...
    upstatLog(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // console.log(post);
            const user = yield database_1.default.query('SELECT id FROM vh_user_admin WHERE id = ?', [id]);
            yield database_1.default.query('UPDATE vh_user_admin set setup = ? WHERE id = ?', [_user.setup, id]);
            if (user == "") {
                res.status(404).json({ text: "User doesn't exists" });
            }
            else {
                res.json({ message: 'The User log was Update' });
            }
        });
    }
    password(req, res) {
